...
`isPresent` настройка у `@FlagCuttable` (которая раньше не была документирована) - будет удалена в ближайшем большом обновлении.
...
```
7. Анти-флаги - Возможность указать `!<flag>` в `@FlagCuttable`, `Inject.flag`. Например `@FlagCuttable("!server")` как замена не документированному `isPresent`
