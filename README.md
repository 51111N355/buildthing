...
## 🚨 Важные изменения
В последней версии BuildThing `@InjectXXX` аннотации были заменены на `Inject.xxx` методы. 

Все вызовы на `Inject.xxx` будут заменяться во время сборки на значения просчитанные из аргументов + конфигурации.

Причинами такого переезда есть поддержка не `public static final` полей + через методы никогда не будет случаться Inline'инг значений Java компилятором, что было очень неприятной особенностью Inject'инга значений аннотациями раньше. 
